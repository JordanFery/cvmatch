...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-10 px-4 text-left align-middle text-xs font-medium text-muted-foreground uppercase tracking-wide whitespace-nowrap",
        className,
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td data-slot="table-cell" className={cn("px-4 py-2.5 align-middle whitespace-nowrap", className)} {...props} />
  );
}
